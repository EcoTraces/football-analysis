...
| 7. Data normalization | 🟡 Partial | Reference-data upsert (country/competition/season/team) by external id done. `competition_type` (league/cup) is now correctly classified for football-data.org, whose `/matches` response includes it (`normalizeCompetitionType()`, `referenceDataService.ts`) — api-football's `/fixtures` endpoint still doesn't send this at all (would need a separate `/leagues` call this MVP doesn't make), so it still always defaults to `'league'` there. Note this only classifies newly-created competition rows — `findOrCreateByExternalRef`'s "find" path never updates an existing row's fields, so a competition already created before this fix keeps whatever type it started with. Team nationality remains genuinely unset by design, not a bug: neither provider's fixtures payload includes a team's own country (only the competition's), and inferring one from the other would be actively wrong for continental competitions — a dedicated team-info sync (api-football has a `/teams` endpoint with this data; football-data.org's `/teams` endpoint would need checking) is the real fix, not yet built |
